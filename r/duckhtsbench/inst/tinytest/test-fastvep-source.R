...
local({
  ...
  extension_hash <- hash(extension)
  expect_equal(expected[["extension_sha256"]], extension_hash)
  expect_equal(read_map(expected[["source_map_sha256"]], input_hash, extension_hash), result)
  ...
  expect_error(read_map(expected_extension_sha256 = strrep("0", 64L)), "extension differs")
  expect_error(read_map(expected_extension_sha256 = "invalid"), "must be SHA256")
  other_extension <- file.path(work, "other.duckdb_extension")
  writeLines("different extension bytes", other_extension)
  expect_error(stage(repo, other_extension), "extension differs")
