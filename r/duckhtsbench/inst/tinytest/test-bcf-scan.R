...
test_bcf_scan_stage <- function() {
  ...
  retained <- c(paths[[1]], paste0(paths[[1]], ".provenance.tsv"))
  before <- list(md5 = tools::md5sum(retained), metadata = file.info(retained)[, c("size", "mtime")])
  expect_error(duckhts_bench_stage_bcf_scan(directory), pattern = "identity does not match")
  expect_identical(readLines(paths[[1]]), "poisoned cached copy")
  expect_identical(list(md5 = tools::md5sum(retained),
    metadata = file.info(retained)[, c("size", "mtime")]), before)
