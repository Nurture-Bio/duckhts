...
duckvep_fastvep_csq_query <- function(con, input, fields, source_map = "") {
  ...
  if (nzchar(source_map)) {
    ...
    source_cte <- paste0(", fastvep_extract_source AS (
    ...
    identity <- "s.record_index, s.alt_index, "
    identity_guard <- " WHEN coalesce(s.source_matches, 0) != 1
      THEN error('CSQ output has unknown or ambiguous physical ALT identity')"
  ...
  paste0("WITH records AS (
    SELECT *, regexp_extract(INFO, '(?:^|;)CSQ=([^;]*)', 1) AS csq,
      length(regexp_extract_all(INFO, '(?:^|;)CSQ=')) AS csq_keys FROM ", raw,
    "), annotations AS (
      SELECT CHROM, POS, ID, REF, ALT, csq_keys,
        string_split(UNNEST(string_split(csq, ',')), '|') AS values FROM records
    )", source_cte, " SELECT ", identity, values, " FROM annotations a", source_join,
    " WHERE CASE WHEN csq_keys != 1 OR length(values) != ", length(header$fields),
    " THEN error('missing/duplicate CSQ or field count differs from its header')",
    identity_guard, " ELSE true END")
}
...
duckvep_fastvep_extract_csq <- function(con, input, table, fields, source_map = "") {
  ...
  query <- duckvep_fastvep_csq_query(con, input, fields, source_map)
  DBI::dbExecute(con, paste("CREATE TEMP TABLE", DBI::dbQuoteIdentifier(con, table), "AS", query))
  invisible(table)
}
...
main <- function() {
  ...
  query <- duckvep_fastvep_csq_query(con, opt$input, duckvep_fastvep_fields("vep_csq"), opt$source_map)
  DBI::dbExecute(con, paste0("COPY (", query, ") TO ", DBI::dbQuoteString(con, opt$output),
