...
DUCKDB_EXTENSION_ENTRYPOINT(duckdb_connection connection,
    ...
    if (!run_sql_or_fail(connection,
        ...
        "__dht_stored_check AS MATERIALIZED (SELECT count(*) > 0 "
        "AND count(sketch) = count(*) "
        "AND count(DISTINCT sketch.sample_id) = count(*) "
        "AND count(DISTINCT struct_pack(min_depth := sketch.min_depth, "
        "min_het_balance := sketch.min_het_balance, "
        "hom_balance_cutoff := sketch.hom_balance_cutoff)) = 1 AS valid, "
        ...
        "(SELECT min_depth FROM __dht_stored_check), "
        "(SELECT min_het_balance FROM __dht_stored_check), "
        "(SELECT hom_balance_cutoff FROM __dht_stored_check), max_sites)), "
        ...
        "SELECT (SELECT valid FROM __dht_stored_check) "
